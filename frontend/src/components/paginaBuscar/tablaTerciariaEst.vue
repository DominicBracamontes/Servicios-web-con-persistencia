...
const fetchInscripciones = async (matricula) => {
  ...
  try {
    ...
    const uniqueAsignaturas = [...new Set(result.data.map(i => i.asignatura.clave))];
    
    const results = await Promise.all(
      uniqueAsignaturas.map(async clave => ({
        clave,
        docentes: await fetchContratos(clave)
      }))
    );
    
    docentesPorAsignatura.value = results.filter(item => item.docentes.length > 0);
